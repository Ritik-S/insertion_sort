...
void insertion_sort(int a[], int n)//a is for array and n is size of array
{
    int i, value , j; 
    for(int i = 1 ;i < n ; ++i)
      {
...
//function to print time taken by insertion_sort to sort different array values with different sizes
void generate_report(int input_size , int increment )
{
      ...
      for(int i = 0;i < 8 ; ++i )
      {
	      ...
	      //variable start stores time before sorting array using insertion sort
	      long long start =te.tv_sec * 1000LL +te.tv_usec /1000;
	      insertion_sort( arr, size );
	      gettimeofday(&te, NULL);
	      //variable end stores time after sorting array using insertion sort
